...
export class Contract {
    ...
    async changeStatusToActiveContract() {
        if (!this.activeEmployeeName && this.createdEmployeeName) {
            this.activeEmployeeName = this.createdEmployeeName;
        }
    ...
    private activeContractorsSearch() {
        return this.page.getByRole('searchbox', { name: 'Username' })
            .or(this.page.getByPlaceholder(/Search by employee/i))
            .or(this.page.getByRole('searchbox'))
            .or(this.page.getByPlaceholder(/search|username/i));
    }

    private async ensureHrSession() {
        await this.page.bringToFront();
        ...
        if (!hrUsername || !hrPassword) {
            throw new Error(
                'HR session expired. Set HR_USERNAME and HR_PASSWORD in .env, or run the full Contract suite from TC01.',
            );
        }
    ...
    async openActiveContractors() {
        ...
        const contractorsTab = this.page.getByText(/Contractors\s*\(\d+\)/).filter({ visible: true })
            ...
            .or(this.page.getByRole('link', { name: /Contractors/i }).filter({ visible: true }));
        await contractorsTab.first().waitFor({ state: 'visible', timeout: 15000 });
        await contractorsTab.first().click();
        await this.page.waitForTimeout(1500);

        if (!(await this.activeContractorsSearch().first().isVisible({ timeout: 5000 }).catch(() => false))) {
            await this.page.goto('/employee-management/active/contract', { waitUntil: 'domcontentloaded' }).catch(() => { });
            await this.page.waitForTimeout(1500);
        }

        await expect(this.activeContractorsSearch().first()).toBeVisible({ timeout: 20000 });
    }

    async searchActiveContractEmployee(employeeName: string) {
        await this.ensureHrSession();

        const targetName = this.activeEmployeeName || this.createdEmployeeName || employeeName;
        const onActiveContractors = await this.page.getByText(/Active Contractors/i).first()
            .isVisible({ timeout: 2000 }).catch(() => false);
        const searchVisible = await this.activeContractorsSearch().first()
            .isVisible({ timeout: 2000 }).catch(() => false);

        if (!onActiveContractors || !searchVisible) {
            await this.openActiveContractors();
        }

        const search = this.activeContractorsSearch();
        ...
        await expect(employee).toBeVisible({ timeout: 60000 });
